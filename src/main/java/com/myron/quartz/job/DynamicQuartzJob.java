...
public class DynamicQuartzJob extends QuartzJobBean {  
    ...
    protected void executeInternal(JobExecutionContext jobexecutioncontext) throws JobExecutionException {  
		...
		try {
			...
			logger.info("dynamic invoke {}.{}() --- {}", serviceImpl.getClass().getName(), methodName, jobDetail.getDescription());
			method.invoke(serviceImpl, arguments);
		} catch (NoSuchMethodException | SecurityException
				| IllegalAccessException | IllegalArgumentException
				| InvocationTargetException e) {
			logger.error("reflect invoke service method error:" + e.getCause().toString());
		}
    }  
